...
def missingness_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #Return a DataFrame with columns: column, missing_rate, missing_count
    ...
    miss_df = miss_df.sort_values(by="missing_rate", ascending=False)
...
def multiple_linear_regression(
    ...
    #Fit the model using least squares: y = intercept + b1*x1 + b2*x2 + ...
    import statsmodels.api as sm
    X = sm.add_constant(X)
    model = sm.OLS(y, X).fit()

    #Return a JSON-safe dictionary containing: outcome, predictors, n_rows_used, r_squared, adj_r_squared, intercept, coefficients (dict)
